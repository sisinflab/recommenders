...
public class BPRMF {
    ...
    public void updateFactors(int u, int i, int j, boolean update_u, boolean update_i, int it){
        ...
        // update bias terms
        if (update_i) {
            double d = z - biasRegularization * itemBias_i;
            dataModel.update_i_bias(i,itemBias_i + learningRate * d);
        }
        if (update_j){
            double d = -z - biasRegularization * itemBias_j;
            dataModel.update_i_bias(j,itemBias_j + learningRate * d);
        }
    ...
    public double loss(){  // TODO da controllare con mymedialite ComputeObjective. non necessaria per la computazione ma solo per visualizzazione
        ...
        for(Triple<Integer,Integer,Integer> uij:lossSamples) {
            ...
            double itemBias_i = dataModel.getItemBias(i);
            double itemBias_j = dataModel.getItemBias(j);
            complexity += userRegularization* dotProduct(userFactors,userFactors); // TODO euclidean norm vedi mymedialite
